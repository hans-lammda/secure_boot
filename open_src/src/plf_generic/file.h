...
#define __PLF_FILE_HEADER__ "$HeadURL: https://cm-ext.dev.oniteo.com/svn/nanodev/packages/plf_generic/trunk/plf_generic/file.h $ $Revision: 24731 $"
...
/*! \brief read attributes  if path is regular file    
    \param[in/out] parameter  structure     
    \retval PLF_FILE_OK 
    \retval PLF_FILE_ATTR_FAIL
    ...
    This functions collects parameters from the filesystem such as 
    permission , size etc.  

*/
int  plf_attr_file(file_io in);

/*! \brief control filesystem  
    \param[in/out] parameter  structure     
    \param[in] data opaque structure      
    \retval PLF_FILE_OK 
    \retval PLF_FILE_ATTR_FAIL


    This functions allows passing low level 
    parameters from application level to 
    underlying driver. 
...
/*! \brief load file     
    ...
    \param [in] len  size to load relative start  
    \param [in] progress callback function  
    \param [in] data low level file system config 

    \retval pointer to file_io struct
    \retval NULL 
    \param[in/out] parameter  structure     
